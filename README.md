...
- **Security headers**: CSP (nonce-based `script-src`, set per-request in `middleware.ts`), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `Permissions-Policy` (`next.config.mjs` + `middleware.ts`).
